...
/// 2. Service Account credentials (via JSON file)
/// 3. External Account credentials for Workload Identity Federation (via JSON file)
/// 4. Application Default Credentials (ADC) from environment or metadata server
...
pub enum GcpAuthenticator {
    // Using AccessTokenCredentials which provides direct access to tokens
...
impl GcpAuthenticator {
    ...
    /// Supports both service account and external account (workload identity) credentials.
    async fn from_file(path: &str, scopes: &[&str]) -> crate::Result<Self> {
        ...
        // Temporarily set GOOGLE_APPLICATION_CREDENTIALS for this build
        // google-cloud-auth 1.6 reads from this environment variable
    ...
    /// Create authenticator using Application Default Credentials (ADC).
    /// This will check for credentials in this order:
    /// 1. GOOGLE_APPLICATION_CREDENTIALS environment variable
    /// 2. gcloud auth application-default credentials
    /// 3. Metadata server (for GCE, GKE, Cloud Run, etc.)
    ...
    /// Get the current authorization token synchronously.
    /// For OAuth credentials, this fetches from the internal cache managed by google-cloud-auth.
    pub fn make_token(&self) -> Option<String> {
        match self {
            Self::Credentials(creds) => {
                // We need to block on the async token fetch
                // google-cloud-auth handles caching internally, so this is efficient
    ...
    /// Modify URI to include API key if using API key authentication.
    ...
    /// Spawn a background task to periodically refresh the token.
    /// The google-cloud-auth library handles token refresh internally,
    /// but we trigger periodic refreshes to ensure tokens stay fresh.
        ...
        match self {
            Self::Credentials(creds) => {
                loop {
                    ...
                    // Fetch a new token to ensure it's refreshed
                    // google-cloud-auth handles the actual refresh logic
...
/// RAII guard to temporarily set an environment variable
struct ScopedEnv {
    key: &'static str,
...
// Re-export common scopes for convenience
pub mod scopes {
    /// Cloud Storage scope for read/write access
    ...
    /// Cloud Monitoring (Stackdriver) scope
    ...
    /// Full cloud platform scope (use sparingly)
